...
int main(int argc, char* argv[])
{
	...
	if(argc < 5)
	{
        ...
        cerr << "                  <format> {registered} {enable} -ini:<file>\n";
		cerr << "\n";
		...
		cerr << "\n";
		cerr << "for -ini file parameter (optional) enter :\n";
		cerr << "  file name\n";
		cerr << "\n";
    ...
    if(argc >= 6)
    {
        for(arg_i = 5; arg_i < argc; arg_i++)
        {
            //1. Is it likely -ini:x switch?
            //2. Is it likely registered or enable option?
            if(strlen(argv[arg_i]) >= 6)
            {
                //Too lazy to check for mixed character cases.
                if(strncmp(argv[arg_i], "-ini:", 5) == 0)
                {
                    ini_file_name = argv[arg_i] + 5;

                    //No path in file name?
                    if(ini_file_name.find_first_of(":\\") == std::string::npos)
                    {
                        //Make it a relative current path (a requirement for some stupid m$ functions).
                        ini_file_name = ".\\" + ini_file_name;
                    }
                }
            }
            else
            if(strlen(argv[arg_i]) == 1)
            {
                if((argv[arg_i][0] == 'r') || (argv[arg_i][0] == 'R') || (argv[arg_i][0] == 'e') || (argv[arg_i][0] == 'E'))
                {
                    if(option_1 == 0)
                    {
                        option_1 = argv[arg_i][0];
                    }
                    else
                    if(option_2 == 0)
                    {
                        option_2 = argv[arg_i][0];
                    }
                }
            }
        }
	...
	int rom_size = (int) pow((double)2,addr_bits);
    ...
    else
    {
        ...
        //Copy RAM buffer to mem buffer for romgen.
        for(addr = 0; addr < rom_size; addr++)
        {
            ...
            mem[addr] = (unsigned char)RAM[addr];
